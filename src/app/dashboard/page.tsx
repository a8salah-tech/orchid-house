...
function EmployeeDashboard({ employee }: { employee: any }) {
  ...
  const MY_LINKS = [
    { icon: '🏧', label: 'الكاشير',     path: '/dashboard/cashier',          show: ['cashier','admin','branch_manager'] },
    { icon: '👨‍🍳', label: 'المطبخ',    path: '/dashboard/kitchen',           show: ['kitchen_supervisor','admin'] },
    { icon: '🍰', label: 'الحلويات',    path: '/dashboard/desserts',          show: ['employee','admin'] },
    { icon: '☕', label: 'البار',       path: '/dashboard/bar',              show: ['bar_supervisor','admin'] },
    { icon: '🪑', label: 'الطاولات',   path: '/dashboard/tables',           show: ['hall_supervisor','cashier','admin'] },
    { icon: '💰', label: 'راتبي',       path: '/dashboard/hr/payroll',       show: ['employee','cashier','kitchen_supervisor','hall_supervisor','bar_supervisor','admin'] },
    { icon: '⏰', label: 'حضوري',       path: '/dashboard/hr/attendance',    show: ['employee','cashier','kitchen_supervisor','hall_supervisor','bar_supervisor','admin'] },
    { icon: '📋', label: 'طلباتي',      path: '/dashboard/hr/requests',      show: ['employee','cashier','kitchen_supervisor','hall_supervisor','bar_supervisor','admin'] },
    { icon: '🕐', label: 'الشيفتات',   path: '/dashboard/hr/shifts',        show: ['employee','cashier','kitchen_supervisor','hall_supervisor','bar_supervisor','admin'] },
    ...
    <div style={{ fontFamily: 'Tajawal, sans-serif', direction: 'rtl', color: S.white }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${S.navy2},${S.navy3})`, borderRadius: 20, border: `1px solid ${S.border}`, padding: '28px 32px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 20, animation: 'fadeUp .4s ease' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 14, color: S.muted }}>{role.label}{employee?.department ? ` · ${employee.department}` : ''}</div>
        ...
        <div style={{ marginRight: 'auto', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 11, color: S.muted }}>الوقت الحالي</div>
        </div>
      </div>
      ...
      <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, padding: '20px 22px', marginBottom: 20 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: S.white, marginBottom: 16 }}>⚡ وصول سريع</div>
      ...
      <div style={{ background: S.gold3, border: `1px solid ${S.gold}40`, borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: S.gold, marginBottom: 2 }}>مرحباً بك في Orchid House</div>
          <div style={{ fontSize: 12, color: S.muted }}>لديك أي استفسار؟ تواصل مع المدير المباشر. نتمنى لك يوم عمل موفق!</div>
        </div>
      </div>
    </div>
  )
...
function AdminDashboard() {
  ...
  const QUICK_LINKS = [
    { icon: '🏧', label: 'الكاشير', path: '/dashboard/cashier', color: S.gold, bg: S.gold3 },
    { icon: '👨‍🍳', label: 'المطبخ', path: '/dashboard/kitchen', color: S.amber, bg: S.amberB },
    { icon: '🪑', label: 'الطاولات', path: '/dashboard/tables', color: S.blue, bg: S.blueB },
    { icon: '📖', label: 'المنيو', path: '/dashboard/menu/items', color: S.teal, bg: S.tealB },
    { icon: '🏭', label: 'المستودع', path: '/dashboard/warehouse', color: S.purple, bg: S.purpleB },
    { icon: '🛒', label: 'المشتريات', path: '/dashboard/purchases', color: S.green, bg: S.greenB },
    { icon: '👥', label: 'العملاء', path: '/dashboard/customers', color: S.blue, bg: S.blueB },
    { icon: '📅', label: 'الحجوزات', path: '/dashboard/bookings', color: S.purple, bg: S.purpleB },
    { icon: '🤝', label: 'الموردون', path: '/dashboard/suppliers', color: S.amber, bg: S.amberB },
    { icon: '🎫', label: 'الكوبونات', path: '/dashboard/coupons', color: S.red, bg: S.redB },
    { icon: '🎁', label: 'الولاء', path: '/dashboard/loyalty', color: S.gold, bg: S.gold3 },
    { icon: '👷', label: 'الموظفون', path: '/dashboard/hr/employees', color: S.teal, bg: S.tealB },
    { icon: '📊', label: 'التقارير اليومية', path: '/dashboard/reports/daily', color: S.green, bg: S.greenB },
    { icon: '📈', label: 'تحليل التكاليف', path: '/dashboard/reports/costs', color: S.purple, bg: S.purpleB },
    { icon: '📋', label: 'شجرة الحسابات', path: '/dashboard/accounting/chart', color: S.blue, bg: S.blueB },
    { icon: '💸', label: 'سندات القيد', path: '/dashboard/accounting/entries', color: S.amber, bg: S.amberB },
    { icon: '📲', label: 'الإشعارات', path: '/dashboard/notifications', color: S.teal, bg: S.tealB },
    { icon: '🔐', label: 'الصلاحيات', path: '/dashboard/settings/permissions', color: S.muted, bg: S.card },
  ]
  ...
  return (
    <div style={{ fontFamily: 'Tajawal, sans-serif', color: S.white, direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
            ...
            <div>
              <h1 style={{ fontSize: 24, fontWeight: 900, color: S.white, marginBottom: 2 }}>لوحة التحكم الرئيسية</h1>
              <div style={{ fontSize: 13, color: S.muted }}>{now.toLocaleDateString('ar-SA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} · {now.toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}</div>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {stats!.waiter_calls > 0 && (
            <div style={{ background: S.redB, border: `1px solid ${S.red}`, borderRadius: 12, padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 8, animation: 'pulse 1.5s ease infinite' }}>
              ...
              <span style={{ color: S.red, fontWeight: 700, fontSize: 13 }}>{stats!.waiter_calls} طلب ويتر</span>
            </div>
          )}
          {stats!.pending_bookings > 0 && (
            <div style={{ background: S.amberB, border: `1px solid ${S.amber}`, borderRadius: 12, padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 18 }}>📅</span>
              <span style={{ color: S.amber, fontWeight: 700, fontSize: 13 }}>{stats!.pending_bookings} حجز معلق</span>
            </div>
          )}
          {stats!.low_stock > 0 && (
            <div style={{ background: S.purpleB, border: `1px solid ${S.purple}`, borderRadius: 12, padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ color: S.purple, fontWeight: 700, fontSize: 13 }}>{stats!.low_stock} منتج منخفض</span>
            </div>
          )}
          <button onClick={fetchStats} style={{ padding: '8px 16px', borderRadius: 12, border: `1px solid ${S.border}`, background: S.card, color: S.muted, cursor: 'pointer', fontSize: 13, fontFamily: 'Tajawal, sans-serif' }}>🔄 تحديث</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 14, marginBottom: 24 }}>
        {[
          { icon: '💰', label: 'إيرادات اليوم', value: stats!.revenue_today, prefix: 'MYR ', decimals: 2, color: S.gold, bg: S.gold3, sub: `${stats!.paid_today} طلب مدفوع`, path: '/dashboard/reports/daily' },
          { icon: '📋', label: 'طلبات نشطة', value: stats!.active_orders, color: S.amber, bg: S.amberB, sub: `${stats!.orders_today} طلب اليوم`, path: '/dashboard/cashier' },
          { icon: '🪑', label: 'الطاولات المشغولة', value: occupancyPct, suffix: '%', color: occupancyPct > 70 ? S.red : S.green, bg: occupancyPct > 70 ? S.redB : S.greenB, sub: `${stats!.occupied_tables} من ${stats!.total_tables} طاولة`, path: '/dashboard/tables' },
          { icon: '📅', label: 'حجوزات اليوم', value: stats!.bookings_today, color: S.purple, bg: S.purpleB, sub: `${stats!.pending_bookings} معلق`, path: '/dashboard/bookings' },
          { icon: '👥', label: 'إجمالي العملاء', value: stats!.total_customers, color: S.blue, bg: S.blueB, sub: `${stats!.total_employees} موظف نشط`, path: '/dashboard/customers' },
          { icon: '📈', label: 'إيرادات الأسبوع', value: stats!.revenue_week, prefix: 'MYR ', decimals: 0, color: S.teal, bg: S.tealB, sub: `الشهر: MYR ${stats!.revenue_month.toFixed(0)}`, path: '/dashboard/reports/monthly' },
        ].map((kpi, i) => (
          <div key={i} onClick={() => router.push(kpi.path)}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 11, color: kpi.color, background: `${kpi.color}20`, borderRadius: 20, padding: '2px 10px', fontWeight: 700 }}>مباشر</div>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${S.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: S.white }}>📋 الطلبات النشطة</div>
            <button onClick={() => router.push('/dashboard/cashier')} style={{ fontSize: 12, color: S.gold, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'Tajawal, sans-serif' }}>عرض الكل ←</button>
          </div>
          <div style={{ padding: '8px 0', maxHeight: 320, overflowY: 'auto' }}>
            {stats!.recent_orders.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 30, color: S.muted, fontSize: 13 }}>✅ لا توجد طلبات نشطة</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, padding: '20px 22px' }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: S.white, marginBottom: 16 }}>🪑 إشغال الطاولات</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: S.muted }}>
                  <span>0</span><span>{stats!.total_tables} طاولة</span>
                </div>
              </div>
              <div style={{ textAlign: 'center', minWidth: 70 }}>
                <div style={{ fontSize: 32, fontWeight: 900, color: occupancyPct > 70 ? S.red : S.green }}>{occupancyPct}%</div>
                <div style={{ fontSize: 11, color: S.muted }}>{stats!.occupied_tables} مشغول</div>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 14 }}>
              {[
                { label: 'مشغول', value: stats!.occupied_tables, color: S.red },
                { label: 'متاح', value: stats!.total_tables - stats!.occupied_tables, color: S.green },
                { label: 'إجمالي', value: stats!.total_tables, color: S.muted },
              ].map((t, i) => (
          ...
          <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, padding: '20px 22px', flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: S.white, marginBottom: 14 }}>🔥 أكثر الأصناف طلباً (أسبوع)</div>
            {stats!.top_items.length === 0 ? (
              <div style={{ textAlign: 'center', color: S.muted, fontSize: 13, padding: 20 }}>لا توجد بيانات</div>
      ...
      <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, padding: '20px 22px', marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: S.white }}>💰 ملخص الإيرادات</div>
          <button onClick={() => router.push('/dashboard/reports/monthly')} style={{ fontSize: 12, color: S.gold, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'Tajawal, sans-serif' }}>التقرير الشهري ←</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
          {[
            { label: 'اليوم', value: stats!.revenue_today, color: S.gold },
            { label: 'الأسبوع', value: stats!.revenue_week, color: S.green },
            { label: 'الشهر', value: stats!.revenue_month, color: S.blue },
          ].map((r, i) => (
      ...
      {/* Quick Access */}
      <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, padding: '20px 22px' }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: S.white, marginBottom: 18 }}>⚡ الوصول السريع</div>
