...
function ShiftReportModal({ orders, waste, shiftStart, onClose }: {
  orders: BarOrder[]; waste: WasteItem[]; shiftStart: Date | null; onClose: () => void
}) {
  const now = new Date()
  ...
  function printReport() {
    ...
    const rows = orders.map((o, idx) => `
      <tr>
        ...
        <td>${o.order_items.filter(i=>i.destination==='bar').map(i => `${i.menu_items?.name_en||i.menu_items?.name} ×${i.quantity}`).join(', ')}</td>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.9)', zIndex: 500, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 20, overflowY: 'auto' }}>
      <div style={{ background: S.navy2, borderRadius: 20, border: `1px solid ${S.border}`, width: '100%', maxWidth: 900, padding: 28, margin: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h2 style={{ color: S.white, fontSize: 18, fontWeight: 800, marginBottom: 4 }}>🥤 Bar Shift Report</h2>
              ...
              <tbody>
                {orders.map((o, i) => (
                  <tr key={o.id}>
                    ...
                    <td style={tdS}>{o.order_items.filter(i => i.destination === 'bar').map(i => `${i.menu_items?.name_en || i.menu_items?.name} ×${i.quantity}`).join(', ')}</td>
...
export default function BarPage() {
  ...
  const [allShiftOrders, setAllShiftOrders] = useState<BarOrder[]>([])
  ...
  // ✅ جديد: جلب طلبات البار ليوم محدد من الأرشيف (كل الحالات، مش النشطة بس) مع فلترة الفرع
  const fetchArchive = useCallback(async (date: string) => {
    ...
    let dayOrders = ((data as any) || []).map((o: any) => ({
      ...o,
      order_items: o.order_items.filter((i: any) => i.destination === 'bar'),
  ...
  // ✅ Fix: الجرس كان بيدق عند أي تغيير في جدول orders بالكامل (حتى لو الطلب مفيهوش أي صنف بار خالص،
  // زي طلب أكل بس) فيبان إشعار "طلب جديد" من غير ما يظهر أي طلب فعلي في الشاشة. دلوقتي بنتتبّع IDs
  // أصناف البار الظاهرة فعليًا، ولا نشغّل الجرس إلا لو ظهر صنف بار جديد فعلاً لم نكن نعرفه من قبل
  ...
  const fetchOrders = useCallback(async () => {
    ...
    `)
      // ✅ Fix: بنجيب الطلبات الملغاة كمان (مش بس confirmed/preparing/ready) - عشان تفضل ظاهرة في البار بدل ما تختفي فورًا
      .in('status', ['confirmed', 'preparing', 'ready', 'cancelled']).order('created_at', { ascending: false }) // ✅ Fix: الطلب الأحدث دلوقتي يظهر أول واحد فوق

    let filtered = ((data as any) || []).map((o: BarOrder) => ({
      ...o,
      order_items: o.order_items.filter(i => i.destination === 'bar'),
    })).filter((o: BarOrder) => {
      if (o.order_items.length === 0) return false
      // ✅ جديد: الطلب الملغي بالكامل يفضل ظاهر بس لمدة 20 دقيقة من لحظة إلغائه، عشان البار ياخد باله، وبعدها يختفي طبيعي (يفضل في الأرشيف)
    ...
    filtered = filtered.sort((a: BarOrder, b: BarOrder) => {
    ...
    if (!isAdmin && myBranchId) {
      filtered = filtered.filter((o: BarOrder) => o.tables?.branch_id === myBranchId)
    }

    // ✅ Fix: نشغّل الجرس فقط لو ظهر صنف بار جديد فعلاً (طلب جديد أو جولة إضافية) لم يكن معروضًا قبل كده،
    // مش عند أي تغيير في جدول orders حتى لو مالوش علاقة بالبار خالص
    const currentItemIds = new Set<string>()
    filtered.forEach((o: BarOrder) => o.order_items.forEach(i => currentItemIds.add(i.id)))
    if (hasLoadedOnceRef.current) {
      let hasNewItem = false
      currentItemIds.forEach(id => { if (!knownItemIdsRef.current.has(id)) hasNewItem = true })
      if (hasNewItem) {
        setNotif(true)
        playNewOrderBeep()
    ...
    setAllShiftOrders(prev => {
      ...
      const newOrders = filtered.filter((o: BarOrder) => !existing.has(o.id))
  ...
  // Restore shift
  useEffect(() => {
    const s = localStorage.getItem('bar_shift_start')
  ...
  async function markItemReady(itemId: string, orderId: string) {
    ...
    const barItems = (allItems || []).filter(i => i.destination === 'bar')
    const allBarReady = barItems.length > 0 && barItems.every(i => ['ready', 'cancelled', 'returned', 'replaced'].includes(i.status))
    if (allBarReady) {
  ...
  function startShift() {
    ...
    localStorage.setItem('bar_shift_start', now.toISOString())
    setAllShiftOrders([])
    setWaste([])
  }
  ...
  function closeReport() {
    ...
    localStorage.removeItem('bar_shift_start')
  ...
  return (
    <div style={{ minHeight: '100vh', background: S.navy, fontFamily: 'Tajawal, sans-serif', direction: 'ltr' }}>
      ...
      {notif && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 4, background: S.teal, zIndex: 999 }} />}
      ...
      <div style={{ background: S.navy2, borderBottom: `1px solid ${S.border}`, padding: '0 20px', display: 'flex', alignItems: 'center', minHeight: 60, gap: 12, flexWrap: 'wrap', paddingTop: 10, paddingBottom: 10 }}>
        <h1 style={{ color: S.teal, fontSize: 18, fontWeight: 900 }}>🥤 Bar</h1>
        ...
        {viewMode === 'archive' ? (
          <div>
            ...
            {archiveLoading ? (
              ...
              // ✅ نحسب عدد الطلبات المكتملة (كل أصناف البار فيها جاهزة)، والملغية بالكامل، والباقي (لسه شغال وقت انتهاء اليوم)
