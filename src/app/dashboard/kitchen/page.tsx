...
export default function KitchenPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    // فلتر: بس الـ order_items اللي destination = kitchen
  ...
  // Real-time
  useEffect(() => {
    const channel = sb.channel('kitchen-orders')
  ...
  async function markItemReady(itemId: string, orderId: string) {
    ...
    // لو كل items الـ kitchen في هذا الطلب ready → الطلب ready
  ...
  return (
    <div style={{ minHeight: '100vh', background: S.navy, fontFamily: 'Tajawal, sans-serif', direction: 'rtl' }}>
      ...
      {/* Notif pulse */}
      {notif && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 4, background: S.amber, zIndex: 999 }} />
      )}
      ...
      <div style={{ background: S.navy2, borderBottom: `1px solid ${S.border}`, padding: '0 24px', display: 'flex', alignItems: 'center', height: 60, gap: 12, position: 'sticky', top: 0, zIndex: 100 }}>
        <h1 style={{ color: S.amber, fontSize: 20, fontWeight: 900 }}>🍳 شاشة المطبخ</h1>
        <div style={{ color: S.muted, fontSize: 13 }}>{orders.length} طلب قيد التحضير</div>
        <div style={{ marginRight: 'auto', fontSize: 12, color: S.muted }}>
          🟢 متصل · يتجدد تلقائياً
        </div>
      </div>
