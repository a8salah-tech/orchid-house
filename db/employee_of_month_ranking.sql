...
--  وتبقى تتطلّب تقييماً معتمداً للشهر نفسه (مثل السلوك الحالي).
--
--  الدالة تتجاوز RLS (payroll_records مقفول على الأدمن) فتعمل لأي موظف يفتح الصفحة.
--
--  يعتمد على وجود جداول: payroll_months, payroll_records, employee_evaluations,
--  attendance, employees, branches.
--
...
create or replace function app_month_recognition(p_month int, p_year int)
returns table (
  employee_id      uuid,
  name             text,
  name_en          text,
...
language sql stable security definer set search_path = public, pg_temp as $$
  ...
  ev as (
    -- أحدث تقييم معتمد للشهر/السنة لكل موظف
    select distinct on (ee.employee_id) ee.employee_id, ee.total_score
  ...
  pr as (
    select r.employee_id,
           coalesce(r.late_hours, 0)      as late_hours,
           coalesce(r.absence_days, 0)    as absence_days,
           coalesce(r.deduction_2, 0) > 0 as has_ded2
  ...
  scored as (
    select
      ev.employee_id,
      round(ev.total_score, 1)                    as eval_score,
  ...
  where coalesce(e.is_active, true) is not false
...
begin
  ...
  perform * from app_month_recognition(extract(month from current_date)::int,
