...
function AssetFormModal({ asset, onClose, onSaved }: { asset?: FixedAsset | null; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    const payload = { name: name.trim(), name_en: nameEn.trim() || null, category: category.trim() || null, description: description.trim() || null, image_url: imgUrl || null }
    const { error } = asset
...
export default function FixedAssetsPage() {
  ...
  async function fetchAll() {
    setLoading(true)
    const [assetsRes, transfersRes, damageRes] = await Promise.all([
              ...
              {filteredAssets.map(a => (
                <div key={a.id} onClick={() => canManage && setShowAssetForm(a)}
                  ...
                  <div style={{ padding: 14 }}>
                    ...
                    {a.description && <div style={{ fontSize: 11, color: S.muted }}>{a.description}</div>}
                  </div>
                </div>
              ))}
            </div>
