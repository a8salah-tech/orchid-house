...
interface Invoice { id: string; invoice_number: string; invoice_date: string; total_amount: number; status: string; image_url: string; notes: string; warehouse_suppliers?: { name: string }; warehouses?: { name: string }; created_at: string }
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'image',
            source: { type: 'base64', media_type: 'image/jpeg', data: base64Image.split(',')[1] || base64Image }
          },
          {
            type: 'text',
...
أعد الرد بـ JSON فقط بهذا الشكل بدون أي نص إضافي:
{
  ...
  "items": [
    {"name": "اسم الصنف المطابق من القائمة أو كما في الفاتورة", "quantity": 0, "unit_price": 0}
  ],
  "notes": "أي ملاحظات"
}`
          }
...
function NewInvoiceModal({ products, suppliers, units, warehouses, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({
    ...
    invoice_number: '', invoice_date: new Date().toISOString().split('T')[0], notes: '',
  })
  ...
  async function handleAIScan(base64: string) {
    ...
    try {
      ...
      // Auto-fill form
      if (result.invoice_number) setForm(p => ({ ...p, invoice_number: result.invoice_number }))
  ...
  async function save() {
    ...
    try {
      ...
      const { data: inv, error: invErr } = await supabase.from('purchase_invoices').insert([{
        ...form, total_amount: total, image_url: invoiceImage || null, status: 'confirmed',
  ...
  return (
    <>
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: S.navy2, borderRadius: 20, border: `1px solid ${S.border}`, width: '100%', maxWidth: 860, padding: 32, margin: 'auto' }}>

          {/* Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
            <div>
              <h2 style={{ color: S.white, fontSize: 18, fontWeight: 800, marginBottom: 4 }}>🛒 فاتورة مشتريات جديدة</h2>
              <p style={{ fontSize: 12, color: S.muted }}>صوّر الفاتورة وسيقوم الذكاء الاصطناعي باستخراج البيانات تلقائياً</p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            ...
            {/* LEFT: صورة الفاتورة */}
            <div>
              <div style={{ fontSize: 13, color: S.gold, fontWeight: 700, marginBottom: 12 }}>📷 صورة الفاتورة</div>
              ...
              <div
                ...
                style={{
                  ...
                  borderRadius: 14, padding: invoiceImage ? 8 : 32,
              ...
              >
                {invoiceImage ? (
                  <img src={invoiceImage} alt="فاتورة" style={{ maxHeight: 280, maxWidth: '100%', borderRadius: 10, objectFit: 'contain' }} />
                ) : (
                  <div>
                    <div style={{ fontSize: 40, marginBottom: 10 }}>📸</div>
                    <div style={{ fontSize: 14, color: S.white, fontWeight: 600, marginBottom: 6 }}>صوّر أو ارفع الفاتورة</div>
                    <div style={{ fontSize: 12, color: S.muted }}>الذكاء الاصطناعي سيستخرج البيانات تلقائياً</div>
                    <div style={{ marginTop: 14, padding: '8px 20px', background: S.gold3, border: `1px solid ${S.gold}`, borderRadius: 8, display: 'inline-block', fontSize: 12, color: S.gold, fontWeight: 700 }}>
                      اختر صورة
                    </div>
                  </div>
                )}
              ...
              {(scanning || scanProgress) && (
                <div style={{ background: scanning ? S.purpleB : scanProgress.includes('✅') ? S.greenB : S.amberB, border: `1px solid ${scanning ? S.purple : scanProgress.includes('✅') ? S.green : S.amber}`, borderRadius: 10, padding: '10px 14px', fontSize: 12, color: scanning ? S.purple : scanProgress.includes('✅') ? S.green : S.amber, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {items.some(i => i.matched) && (
                <div style={{ background: S.greenB, border: `1px solid ${S.green}`, borderRadius: 10, padding: '10px 14px', marginTop: 10, fontSize: 12 }}>
                  <span style={{ color: S.green, fontWeight: 700 }}>✅ تم مطابقة {matchedCount} من {items.length} صنف تلقائياً</span>
                </div>
              )}

              {invoiceImage && (
                <button onClick={() => { setInvoiceImage(null); setItems([{ product_id: '', product_name: '', quantity: '', unit_price: '', unit_id: '', matched: false }]) }}
                  style={{ marginTop: 8, width: '100%', padding: '8px', borderRadius: 8, border: `1px solid ${S.red}`, background: S.redB, color: S.red, cursor: 'pointer', fontSize: 12, fontFamily: 'Tajawal, sans-serif' }}>
                  🗑️ حذف الصورة وإعادة المحاولة
                </button>
              )}
            </div>
          ...
          <div style={{ marginTop: 24, borderTop: `1px solid ${S.border}`, paddingTop: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1fr 1fr auto', gap: 8, marginBottom: 8 }}>
              {['الصنف', 'الكمية', 'سعر الوحدة', 'الوحدة', ''].map(h => (
                <div key={h} style={{ fontSize: 11, color: S.muted, fontWeight: 700, paddingRight: 4 }}>{h}</div>
              ))}
            </div>
            ...
            {items.map((item, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1fr 1fr auto', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                <div style={{ position: 'relative' }}>
                  <select
                    style={{ ...inp, borderColor: item.matched ? S.green : item.product_id ? S.border : 'rgba(255,255,255,0.10)', paddingRight: item.matched ? '30px' : '14px' }}
                    value={item.product_id}
                    onChange={e => setItem(i, 'product_id', e.target.value)}
                  >
                    <option value="">{item.product_name || 'اختر الصنف'}</option>
                    {products.map(p => <option key={p.id} value={p.id}>{p.name}{p.name_en ? ` — ${p.name_en}` : ''}</option>)}
                  </select>
                  {item.matched && <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: S.green, fontSize: 14 }}>✓</span>}
                </div>
                ...
                <button onClick={() => setItems(p => p.filter((_, idx) => idx !== i))} style={{ background: S.redB, border: `1px solid ${S.red}`, borderRadius: 8, color: S.red, cursor: 'pointer', padding: '8px 10px', fontSize: 14 }}>✕</button>
              </div>
            ))}

            {/* Total */}
            <div style={{ background: S.navy3, borderRadius: 12, padding: '14px 18px', marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: S.muted, fontSize: 14 }}>إجمالي الفاتورة</span>
              <span style={{ color: S.gold, fontSize: 24, fontWeight: 800 }}>{total.toFixed(2)} <span style={{ fontSize: 14, color: S.muted }}>MYR</span></span>
            </div>
          </div>
    ...
    <div style={{ fontFamily: 'Tajawal, sans-serif', direction: 'rtl', color: S.white }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button
          ...
          style={{ padding: '11px 22px', borderRadius: 12, border: `1px solid ${S.gold}`, background: S.gold3, color: S.gold, cursor: 'pointer', fontSize: 14, fontFamily: 'Tajawal, sans-serif', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 28 }}>
        {[
          { label: 'إجمالي الفواتير', value: invoices.length, icon: '🧾', color: S.blue, bg: S.blueB, sub: 'فاتورة' },
          { label: 'مشتريات هذا الشهر', value: monthInvoices.length, icon: '📅', color: S.green, bg: S.greenB, sub: 'فاتورة' },
          { label: 'إجمالي هذا الشهر', value: monthTotal.toFixed(0) + ' MYR', icon: '💰', color: S.gold, bg: S.gold3, sub: '' },
          { label: 'إجمالي كل الفواتير', value: totalAll.toFixed(0) + ' MYR', icon: '📊', color: S.purple, bg: S.purpleB, sub: '' },
          { label: 'عدد الموردين', value: suppliers.length, icon: '🤝', color: S.teal, bg: S.tealB, sub: 'مورد' },
        ].map((s, i) => (
      ...
      {/* ══ AI Banner ══ */}
      <div style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.15), rgba(59,130,246,0.08))', border: `1px solid rgba(139,92,246,0.25)`, borderRadius: 16, padding: '18px 24px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ fontSize: 32 }}>🤖</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: S.white, marginBottom: 4 }}>مسح الفواتير بالذكاء الاصطناعي</div>
          <div style={{ fontSize: 12, color: S.muted }}>صوّر أي فاتورة ورقية وسيقوم الذكاء الاصطناعي باستخراج اسم المورد، التاريخ، رقم الفاتورة، وجميع الأصناف مع الكميات والأسعار تلقائياً ومطابقتها مع منتجات النظام</div>
        </div>
        <button onClick={() => setShowNew(true)} style={{ padding: '10px 20px', borderRadius: 10, border: `1px solid ${S.purple}`, background: S.purpleB, color: S.purple, cursor: 'pointer', fontSize: 13, fontFamily: 'Tajawal, sans-serif', fontWeight: 700, whiteSpace: 'nowrap' }}>
          جرّب الآن ✨
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <input style={{ ...inp, width: 'auto' }} type="month" value={filterMonth} onChange={e => setFilterMonth(e.target.value)} placeholder="تصفية بالشهر" />
        {(search || filterSupplier || filterMonth) && (
          <button onClick={() => { setSearch(''); setFilterSupplier(''); setFilterMonth('') }} style={{ padding: '10px 14px', borderRadius: 10, border: `1px solid ${S.red}`, background: S.redB, color: S.red, cursor: 'pointer', fontSize: 12, fontFamily: 'Tajawal, sans-serif' }}>✕ مسح الفلاتر</button>
        )}
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
              <thead>
                <tr style={{ background: S.navy3 }}>
                  {['رقم الفاتورة', 'المورد', 'المستودع', 'التاريخ', 'الإجمالي', 'الحالة', 'صورة', ''].map(h => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} style={{ textAlign: 'center', padding: 60, color: S.muted }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 6 }}>لا توجد فواتير بعد</div>
                  ...
                  <tr key={inv.id} className="inv-row" style={{ borderBottom: `1px solid ${S.border}`, cursor: 'pointer' }} onClick={() => setSelectedInvoice(inv)}>
                    <td style={{ padding: '14px 16px', color: S.gold, fontWeight: 700, fontSize: 13 }}>{inv.invoice_number || <span style={{ color: S.muted, fontStyle: 'italic' }}>بدون رقم</span>}</td>
                    ...
                    <td style={{ padding: '14px 16px', fontSize: 12, color: S.muted }}>{inv.warehouses?.name || '—'}</td>
                    <td style={{ padding: '14px 16px', fontSize: 12, color: S.muted }}>{inv.invoice_date}</td>
                    <td style={{ padding: '14px 16px', fontWeight: 700, color: S.green, fontSize: 14 }}>{inv.total_amount?.toFixed(2)} <span style={{ fontSize: 11, color: S.muted, fontWeight: 400 }}>MYR</span></td>
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ background: S.greenB, color: S.green, borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>✅ مؤكدة</span>
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      {inv.image_url
                        ? <img src={inv.image_url} alt="فاتورة" style={{ width: 38, height: 38, objectFit: 'cover', borderRadius: 8, cursor: 'pointer', border: `1px solid ${S.border}` }} onClick={(e) => { e.stopPropagation(); window.open(inv.image_url, '_blank') }} />
      ...
      {selectedInvoice && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: S.navy2, borderRadius: 18, border: `1px solid ${S.border}`, width: '100%', maxWidth: 520, padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ color: S.gold, fontSize: 17, fontWeight: 700 }}>🧾 {selectedInvoice.invoice_number || 'فاتورة'}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                { label: 'المستودع', value: selectedInvoice.warehouses?.name || '—' },
                { label: 'التاريخ', value: selectedInvoice.invoice_date },
                { label: 'إجمالي الفاتورة', value: `${selectedInvoice.total_amount?.toFixed(2)} MYR`, color: S.gold },
              ...
              {selectedInvoice.image_url && (
                <div style={{ marginTop: 8 }}>
                  ...
                  <img src={selectedInvoice.image_url} alt="فاتورة" style={{ width: '100%', borderRadius: 10, cursor: 'pointer', border: `1px solid ${S.border}` }} onClick={() => window.open(selectedInvoice.image_url, '_blank')} />
                </div>
              )}
            </div>
